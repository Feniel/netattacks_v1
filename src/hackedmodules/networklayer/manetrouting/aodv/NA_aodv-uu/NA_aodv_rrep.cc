                           ...
                           struct in_addr orig_addr, u_int32_t life)
{
    ...
    //frreq
    rrep->creator = getAddress().getIPv4();
    ...
    if (isLocalAddress (rrep_orig.s_addr))
    {
        ...
        for(int i=0;i<aodv_delay_dst.size();i++){
            ...
            if(tmp == aodv_delay_dst[i].S_addr){
                simtime_t time_tmp = simTime().dbl() - aodv_delay_time[i];
                //std::cout << time_tmp.dbl() << endl;
                emit(aodv_delay, time_tmp);
                aodv_delay_counter++;
                emit(aodv_total_routes_establisht, aodv_delay_counter);
