...
cMessage *NA_BlackholeAttack::generateAttackMessage(const char *name) {
    ...
    NA_BlackholeMessage *msg = new NA_BlackholeMessage(name);
    msg->setBlackholeAttackProbability(
            par("blackholeAttackProbability").doubleValue());
    msg->setSinkOnlyWhenRouteInTable(
            par("dropOnlyWhenRouteInTable").boolValue());
